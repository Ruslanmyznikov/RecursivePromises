...
function startRecursive(total = 0) {
  return new Promise (resolve => {
    if (!users.length){
      resolve(total)
      return
    ...
    let tenUsersArray = users.splice(0, 10);

    let arrayOfPromises = [];
    // Call processUser(users[i].id)
      for (let i = 0; i < tenUsersArray.length; i++) {
        let temp = processUser(tenUsersArray[i].id);
        arrayOfPromises.push(temp);
      }
